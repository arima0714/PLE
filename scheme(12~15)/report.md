...
`map-tree2` 関数内では `cons`, `car`, `cdr` のそれぞれを用いることなく、 `map-tree` と同じ入力例で同じ出力をしたからである。
...
5. 乗算である場合
5. 巾乗である場合
の全てを計算可能であるということが上から6つの実行例からわかる。また、それらの組み合わせである下2つの実行例はWEB上の出力結果の例と合致しているため、複合的な入力であってもそれに対応しているということがわかるからである。

#### 課題3-2

このプログラムは題意を満たしたプログラムであると考察する。
2個の実行例がともに手計算によって導出したものと一致したからである。微分をする関数はすでに課題3-1で作成したものであるので、この課題にて問題となるのは微分した式を利用することである。二通りの式によって導出することができたため、機能は満たしていると考えたからである。
このプログラムの課題点は与えられた式を微分したものに第二引数を代入するのだが、これを複数回行ってしまっている点である。これを解消するには `let` を用いた局所的な変数を利用することでより簡潔なプログラムになったのではないかと考察する。
